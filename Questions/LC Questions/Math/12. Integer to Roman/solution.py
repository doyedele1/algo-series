'''
    Explanation:
        Note: Knowing which representation is the correct one for a particular number can be difficult.
        There are many possible ways of representing the number 120 for instance.
        ...
        - If there are no special rules, when num = 1500,
            ...
            - Then 1500 % 1000 = 500
            - 500 // 500 = 1. One D in the final result
        ...
        - We converted the symbol-value table to a list of tuples because we need to iterate through the items in a sorted reverse order
    ...
    SC - O(1) since the mappingList doesn't change with the size of the input
'''

class Solution:
    def intToRoman(self, num: int) -> str:
        mappingList = [("I", 1), ("IV", 4), ("V", 5), ("IX", 9), ("X", 10), ("XL", 40),
        ("L", 50), ("XC", 90), ("C", 100), ("CD", 400), ("D", 500), ("CM", 900), ("M", 1000)]

        res = ""
        for symbol, value in reversed(mappingList):
            if num // value:
                count = num // value
                res += (symbol * count)
                num %= value
