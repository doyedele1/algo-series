'''
    ...
    Explanation I: Brute-force solution
        We take all possible step combinations at every step
        climbStairs(i, n) = climbStairs(i + 1, n) + climbStairs(i + 2, n)
        i is the current step (level) and n is the destination step
        
        Base case is if i > n, then return 0 and if i == n, return 1
    ...
    Explanation II: Recursion with Memoization/Caching
        ...
        Instead, we can store the result at each step in memo array and return it when the function is called. i.e. we are storing the result of each of the subproblem
    ...
    Explanation IV: Bottom-up Dynamic Programming Approach
        ...
        dp =   [8   5   3   2   1    1]. At position 3, we can get to "5" 1 + 1 different ways.
                                one  two
        The resulting array looks like a Fibonacci sequence
        We don't need an extra dp array which would cost O(n) space
        If we initialize one and two as 1, we can see that we only need to compute 4 other positions
