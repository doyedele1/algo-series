'''
    Explanation:
        ...
        2. To keep track of all previous travel between any two stations, we create another map called travels. id = stationNames separated by a delimiter, value = tuple of count, total --> [count, total]
        ...
        getAverageTime(A,B) --> combine the two stations and look it up in the travels map = 2 / 1 = 2
        ...
        SC - O(n + m) where n is the number of arrivals and m is the number of averages. n grows larger when we have customers that check in but don't check out - we remove those entries from the arrivals map in the checkout function. m increases the more times the customers check out.
'''
